  ...
  static async getConfig(): Promise<SiteConfig> {
    try {
      ...
      return {
        ...
        paypalClientId: (doc.paypalClientId as string) ?? this.defaultConfig.paypalClientId,
        paypalEnvironment: (doc.paypalEnvironment as 'sandbox' | 'live') ?? this.defaultConfig.paypalEnvironment
      }
    } catch (_error) {
